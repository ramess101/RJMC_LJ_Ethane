...
marker='testing_QPrior'
output=[]
num_trials=10
variables=[0.25,0.5,0.75,1,1.25,1.5,1.75,2,2.25,2.5]
...
for i in range(num_trials):
    ...
    Q_prior=[0,0.216]
    n_iter=2*10**6
    prior_max_values=[variables[i],5,3,5]
    print(prior_max_values)
    trace,logp_trace,percent_deviation_trace, attempt_matrix,acceptance_matrix,prop_sd,accept_vector,alpha_vector=run_RJMC(compound,properties,temp_range,n_points,eps_sig_L_prior_params,Q_prior,prior_type,n_iter,biasing_factor_AUA=0,biasing_factor_UA=0,swap_freq=0.1,prior_max_values=prior_max_values)
    print(marker)
    ...
    trace_equil[:,2:]*=10
    percent_deviation_trace_equil = percent_deviation_trace[t0:]
