...
class RJMC_Simulation():
    ...
    def __init__(self, compound, T_range, properties, n_points, steps, swap_freq,
                 biasing_factor, optimum_matching, tune_freq=100, tune_for=10000):
    ...
    def gen_Tmatrix(self, prior, compound_2CLJ):
        ...
        def obj_AUA(eps_sig_Q): return -self.calc_posterior(prior, compound_2CLJ,
                                                            [0, eps_sig_Q[0], eps_sig_Q[1], eps_sig_Q[2], eps_sig_Q[3]])
        def obj_AUA_Q(eps_sig_Q): return -self.calc_posterior(prior, compound_2CLJ,
                                                              [1, eps_sig_Q[0], eps_sig_Q[1], eps_sig_Q[2], eps_sig_Q[3]])
        def obj_2CLJ(eps_sig_Q): return -self.calc_posterior(prior, compound_2CLJ,
    ...
    def jacobian(self):
        ...
        if self.optimum_matching == 'True':
            jacobian_matrix[0, 1] = (1 / (self.lamda * self.w)) * (self.opt_params_AUA_Q[0] * self.opt_params_AUA_Q[1]
                                                                   * self.opt_params_AUA_Q[2]) / (self.opt_params_AUA[0] * self.opt_params_AUA[1] * self.opt_params_AUA[2])
            jacobian_matrix[1, 0] = self.lamda * (self.opt_params_AUA[0] * self.opt_params_AUA[1] * self.opt_params_AUA[2]) / (
                self.opt_params_AUA_Q[0] * self.opt_params_AUA_Q[1] * self.opt_params_AUA_Q[2])
        ...
        jacobian_matrix[0, 2] = (self.opt_params_UA[0] * self.opt_params_UA[1] * self.opt_params_UA[2]) / \
            (self.opt_params_AUA[0] * self.opt_params_AUA[1] * self.opt_params_AUA[2])
        jacobian_matrix[2, 0] = (self.opt_params_AUA[0] * self.opt_params_AUA[1] * self.opt_params_AUA[2]
    ...
    def refit_prior(self, prior_type='exponential'):
        if prior_type == 'exponential':
            new_prior = fit_exponential_sp(self.trace_model_1)

            Q_prior = [prior_type,new_prior]
        elif prior_type == 'gamma':
            new_prior = fit_gamma_sp(self.trace_model_1)
            Q_prior = [prior_type,new_prior]
        return Q_prior
...
def main():
    ...
    rjmc_simulator = RJMC_Simulation(compound, T_range, properties, n_points, steps,
                                     swap_freq, biasing_factor, optimum_matching)
